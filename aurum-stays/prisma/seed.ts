...
async function main() {
  ...
  for (const p of properties) {
    await prisma.property.upsert({
      where: { slug: p.slug },
      update: { images: p.images, description: p.description, amenities: p.amenities },
      create: p,
    });
  }
  ...
  console.log("Seed complete: admin, user, 6 properties with 5 unique images each + reviews.");
}
